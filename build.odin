...
// Paths configuration
ODIN_VENDOR_PATH :: #config(ODIN_VENDOR_PATH, "C:/Users/anton/odin/vendor")
ODIN_SHARED_PATH :: #config(ODIN_SHARED_PATH, "C:/Users/anton/odin/shared")
RAYLIB_DLL_PATH :: #config(RAYLIB_DLL_PATH, "C:/Users/anton/odin/vendor/raylib/windows")
SHADERCROSS_PATH :: #config(SHADERCROSS_PATH, "C:/Users/anton/shadercross")
...
validate_config :: proc() -> bool {
	if !os.exists(PROJECT_ROOT) {
		logging.log_build_error(
			"Project root '%s' does not exist. Please update the build.odin file.",
			PROJECT_ROOT,
		)
		return false
	}
	...
	if !os.exists(raylib_path) {
		logging.log_build_error(
			"raylib.dll not found at '%s'. Please update the RAYLIB_DLL_PATH in build.odin.",
			raylib_path,
		)
		return false
	}
	...
	if !os.exists(raygui_path) {
		logging.log_build_error(
			"raygui.dll not found at '%s'. Please update the RAYLIB_DLL_PATH in build.odin.",
			raygui_path,
		)
		return false
	}
...
copy_directory :: proc(src, dst: string) {
	...
	copy_cmd := []string {
		...
		"/NJS",
		"/NC",
		"/NS", // Reduce output verbosity
	}

	logging.log_build_info("Copying from %s to %s", src, dst)

	process, err := os.process_start(
		{command = copy_cmd, stdin = os.stdin, stdout = os.stdout, stderr = os.stderr},
	)
...
main :: proc() {
	...
	if !validate_config() {
		logging.log_build_error(
			"Configuration validation failed. Please check the build.odin file.",
		)
		os.exit(1)
	}
